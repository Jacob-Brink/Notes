...
class List
{
    public:
	    ...
	    bool insertAfter(const Item& it, const Item& newIt);
	    bool insertBefore(const Item& it, const Item& newIt);
...
template<class Item>
ostream& operator<<(ostream& out, const List<Item>& oList) {
		oList.writeTo(out, "=");
		return out;
}
...
template<class Item>
bool List<Item>::insertAfter(const Item& it, const Item& newIt) {
	Node* oPtr = myFirst;
	while(oPtr) {
		if (oPtr->myItem == it) {
			Node* newNode = new Node(newIt, oPtr->myNext);
			oPtr->myNext = newNode;
			if(oPtr == myLast) {
				myLast = newNode;
			}
...
template<class Item>
bool List<Item>::insertBefore(const Item& it, const Item& newIt) {
	if (myFirst) {
		if (myFirst->myItem == it) {
			Node* newNode = new Node(newIt, myFirst);
			myFirst = newNode;
			++mySize;
			return true;
		} else {
			...
			while(oPtr) {
				if (oPtr->myItem == it) {
					Node* newNode = new Node(newIt, oPtr);
					iPtr->myNext = newNode;
					++mySize;
					return true;
				}
