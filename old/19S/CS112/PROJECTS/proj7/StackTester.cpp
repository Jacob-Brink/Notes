 ...
 *  dynamically allocated (array) Stack<int> operations
...
void StackTester::runTests() {
	cout << "Testing Stack<int> class..." << endl;
	testConstructor();
	testIsEmpty();
...
void StackTester::testSizeCapacity() {
	...
	//setCapacity to less than initial capacity
	Stack<int> s5(5);
	...
	try {
		s5.setCapacity(4);
		cerr << "\nSet capacity to less than initial\n";
		exit(1);
	} catch (const StackException& se) {
		cout << "asdf" << se << flush;
		cout << " 2b " << flush;
...
void StackTester::testAssignment() {
	...
	assert( s2.myCapacity == 1 );
	cout << " 1 " << flush;
	// minimal non-empty Stack<int> to empty Stack, same capacity
	Stack<int> s3(1);
	s3.push(11);
	Stack<int> s4(1); 
	s4 = s3;
