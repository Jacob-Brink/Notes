...
void PlayListTester::testSave() {
	...
	// create song object for adding and removing
	Song s1("Title", "Artist", 2020);

	// open file to playlist
    PlayList pList("testSongs.txt");

    //alter playList by adding a song unique from any other song, then save
    pList.addSong(s1);
    pList.save();

    //make new playList from same file
    PlayList pList2("testSongs.txt");

    //check if new playList contains unique song and has correct number of songs
    assert(pList2.getNumSongs() == pList.getNumSongs());
    assert(pList2.searchByArtist("Artist").size() == 1);
    cout << " 0 " << flush;
    //To make sure test runs correctly more than once, save original playlist
    pList.removeSong(s1);
    pList.save();
