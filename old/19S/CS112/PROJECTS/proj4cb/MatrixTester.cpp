...
void MatrixTester::runTests() {
	cout << "Running Matrix<int> tests..." << endl;
...
void MatrixTester::testSubscripts() {
	...
	try {
		m2[2][0] = 0;
		cerr << "successfully wrote past last Matrix<int> row";
		exit(1);
	} catch (range_error&) {
		cout << " 5 " << flush;
 	}
	...
	try {
		m2[0][3] = 0;
		cerr << "successfully wrote past last Matrix<int> column";
		exit(1);
	} catch (range_error&) {
		cout << " 6 " << flush;
 	}
...
void MatrixTester::testWriteToStream() {
	...
	Matrix<int> m;
	// read a Matrix<int> whose values we know
	m.readFrom("MatrixTest1.txt");
...
void MatrixTester::testWriteToFile() {
	...
	Matrix<int> m;
	// read in a Matrix<int> whose values we know
	m.readFrom("MatrixTest1.txt");
	// write it to a file
